...
export default dispatch => ({
    ...
    getCurrentCity: (id, force) => {
        ...
        dispatch({
            ...
            payload: store.getState().cities.list.find((currValue) => (currValue.id === id))
        });
    },
    editCity: (data) => {
        ...
        api(`/city/${data.id}`, {
            method: 'PUT',
            body: JSON.stringify({id: data.id, city: data.name, region: data.value})
        }).then(async res => {
            const json = await res.json();
    ...
    removeAttr: (data) => {
        ...
        api(`/attributes/${data.id}/`, {method: 'DELETE', body: JSON.stringify({id: data.id})}).then(async res => {
            dispatch({type: 'DELETE_ATTRS', payload: data});
        });
    },
    addAttr: (data) => {
    ...
    editAttr: (data) => {
        api(`/attributes/${data.id}/`, {
            method: 'PUT',
            body: JSON.stringify({id: data.id, name: data.name, value: data.value})
        }).then(async res => {
            const json = await res.json();
